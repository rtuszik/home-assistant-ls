...
exports.HomeAssistantLanguageService = void 0;
const vscode_languageserver_protocol_1 = require("vscode-languageserver-protocol");
const arrUtils_1 = require("yaml-language-server/out/server/src/languageservice/utils/arrUtils");
const areas_1 = require("./completionHelpers/areas");
const deviceIds_1 = require("./completionHelpers/deviceIds");
const entityIds_1 = require("./completionHelpers/entityIds");
const floors_1 = require("./completionHelpers/floors");
const labels_1 = require("./completionHelpers/labels");
const services_1 = require("./completionHelpers/services");
const domains_1 = require("./completionHelpers/domains");
const uuids_1 = require("./completionHelpers/uuids");
const secrets_1 = require("./completionHelpers/secrets");
const dto_1 = require("./haConfig/dto");
class HomeAssistantLanguageService {
    yamlLanguageService;
    haConfig;
    haConnection;
    definitionProviders;
    ...
    constructor(yamlLanguageService, haConfig, haConnection, definitionProviders, schemaServiceForIncludes, sendDiagnostics, diagnoseAllFiles, configurationService) {
        this.yamlLanguageService = yamlLanguageService;
        this.haConfig = haConfig;
        this.haConnection = haConnection;
        this.definitionProviders = definitionProviders;
        this.schemaServiceForIncludes = schemaServiceForIncludes;
        this.sendDiagnostics = sendDiagnostics;
        this.diagnoseAllFiles = diagnoseAllFiles;
    ...
    findAndApplySchemas = () => {
        try {
            ...
            if (haFiles && haFiles.length > 0) {
                console.log(`Applying schemas to ${haFiles.length} of your configuration files...`);
            }
            ...
            console.error(`Unexpected error updating the schemas, message: ${message}`, error);
        }
        console.log("Schemas updated!");
    };
    getValidYamlTags() {
        const validTags = [];
        for (const item in dto_1.Includetype) {
    ...
    onDocumentChange = (document) => {
        ...
        this.onDocumentChangeDebounce = setTimeout(async () => {
            ...
            if (singleFileUpdate.isValidYaml && singleFileUpdate.newFilesFound) {
                console.log(`Discover all configuration files because ${document.uri} got updated and new files were found...`);
    ...
    onDocumentOpen = async (document) => {
        const diagnostics = await this.getDiagnostics(document);
        this.sendDiagnostics(document.uri, diagnostics);
    };
    getDiagnostics = async (document) => {
        if (!document || document.getText().length === 0) {
            return [];
        }
        const diagnosticResults = await this.yamlLanguageService.doValidation(document, false);
        ...
        for (const diagnosticItem of diagnosticResults) {
            ...
            if (secretStartChar < secretEndChar) {
                const possibleSecret = document.getText(vscode_languageserver_protocol_1.Range.create(startLine, secretStartChar, endLine, secretEndChar));
            ...
            if (inputStartChar < inputEndChar) {
                const possibleInput = document.getText(vscode_languageserver_protocol_1.Range.create(startLine, inputStartChar, endLine, inputEndChar));
            ...
            if (includeStartChar < includeEndChar) {
                const possibleInclude = document.getText(vscode_languageserver_protocol_1.Range.create(startLine, includeStartChar, endLine, includeEndChar));
    ...
    validateEntityIds = async (document) => {
        const diagnostics = [];
        try {
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                for (const propertyName of entityIds_1.EntityIdCompletionContribution.propertyMatches) {
                    ...
                    while ((match = propertyRegex.exec(line)) !== null) {
                        ...
                        if (!entityIds.includes(cleanEntityValue)) {
                            ...
                            const diagnostic = {
                                severity: 2, // Warning
                                range: vscode_languageserver_protocol_1.Range.create(lineIndex, startColumn, lineIndex, endColumn),
    ...
    validateAreaIds = async (document) => {
        const diagnostics = [];
        try {
            ...
            const areaIds = areaCompletions.map(area => area.label);
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                for (const propertyName of areas_1.AreaCompletionContribution.propertyMatches) {
    ...
    validateDeviceIds = async (document) => {
        const diagnostics = [];
        try {
            ...
            const deviceIds = deviceCompletions.map(device => device.label);
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                for (const propertyName of deviceIds_1.DeviceCompletionContribution.propertyMatches) {
    ...
    validateFloorIds = async (document) => {
        const diagnostics = [];
        try {
            ...
            const floorIds = floorCompletions.map(floor => floor.label);
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                for (const propertyName of floors_1.FloorCompletionContribution.propertyMatches) {
    ...
    validateSecrets = async (document) => {
        const diagnostics = [];
        try {
            // Get all available secrets from secrets.yaml file
            const fileAccessor = this.haConfig.getFileAccessor();
            const secretsHelper = new secrets_1.SecretsCompletionContribution(fileAccessor);
    ...
    isInActionContext(lines, currentLineIndex) {
    ...
    validateLabelIds = async (document) => {
        const diagnostics = [];
        try {
            ...
            const labelIds = labelCompletions.map(label => label.label);
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                for (const propertyName of labels_1.LabelCompletionContribution.propertyMatches) {
    ...
    validateActionIds = async (document) => {
        const diagnostics = [];
        try {
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                for (const propertyName of services_1.ServicesCompletionContribution.propertyMatches) {
    ...
    onDocumentSymbol = (document) => {
        if (!document) {
            return [];
        }
        return this.yamlLanguageService.findDocumentSymbols(document, {});
    };
    onDocumentFormatting = async (document, options) => {
        if (!document) {
            return [];
        }
    ...
    onCompletion = async (textDocument, position) => {
        ...
        const currentCompletions = await this.yamlLanguageService.doComplete(textDocument, position, false);
        const additionalCompletions = await this.getServiceAndEntityCompletions(textDocument, position, currentCompletions);
        if (additionalCompletions.length === 0) {
            return currentCompletions;
        }
        return vscode_languageserver_protocol_1.CompletionList.create(additionalCompletions, false);
    };
    onCompletionResolve = async (completionItem) => completionItem;
    onHover = async (document, position) => {
        if (!document) {
            return null;
        }
    ...
    isHoveringOverYamlKey(document, position) {
        try {
            const text = document.getText();
    ...
    async getEntityHoverInfo(document, position) {
        try {
            ...
            const entityContribution = new entityIds_1.EntityIdCompletionContribution(this.haConnection);
            const markedStrings = await entityContribution.getInfoContribution(document.uri, location);
            if (markedStrings && markedStrings.length > 0) {
                const range = vscode_languageserver_protocol_1.Range.create(document.positionAt(start), document.positionAt(end));
                return {
    ...
    async getServiceHoverInfo(document, position) {
        try {
            // Get the word at the position
            const text = document.getText();
            const offset = document.offsetAt(position);
            ...
            const servicesContribution = new services_1.ServicesCompletionContribution(this.haConnection);
            const markedStrings = await servicesContribution.getInfoContribution(document.uri, location);
            if (markedStrings && markedStrings.length > 0) {
                const range = vscode_languageserver_protocol_1.Range.create(document.positionAt(start), document.positionAt(end));
                return {
                    contents: markedStrings,
    ...
    async getTemplateHoverInfo(document, position) {
        try {
    ...
    extractTemplateValue(line, character) {
        const colonIndex = line.indexOf(":");
    ...
    getTemplateValueRange(line, lineNumber, templateValue) {
        const colonIndex = line.indexOf(":");
        const valueStart = colonIndex + 1;
        const trimmedStart = line.substring(valueStart).indexOf(templateValue) + valueStart;
        return vscode_languageserver_protocol_1.Range.create(vscode_languageserver_protocol_1.Position.create(lineNumber, trimmedStart), vscode_languageserver_protocol_1.Position.create(lineNumber, trimmedStart + templateValue.length));
    }
    onDefinition = async (textDocument, position) => {
        if (!textDocument) {
            return undefined;
        }
        const lineOffsets = (0, arrUtils_1.getLineOffsets)(textDocument.getText());
        const start = lineOffsets[position.line];
        const end = lineOffsets[position.line + 1];
    ...
    getSecretsCompletion = async (document, textDocumentPosition) => {
        const lineOffsets = (0, arrUtils_1.getLineOffsets)(document.getText());
        const start = lineOffsets[textDocumentPosition.line];
    ...
    getServiceAndEntityCompletions = async (document, textDocumentPosition, currentCompletions) => {
        ...
        const properties = {};
        properties.areas = areas_1.AreaCompletionContribution.propertyMatches;
        properties.entities = entityIds_1.EntityIdCompletionContribution.propertyMatches;
        properties.floors = floors_1.FloorCompletionContribution.propertyMatches;
        properties.labels = labels_1.LabelCompletionContribution.propertyMatches;
        properties.services = services_1.ServicesCompletionContribution.propertyMatches;
        properties.domains = domains_1.DomainCompletionContribution.propertyMatches;
        properties.uuids = uuids_1.UuidCompletionContribution.propertyMatches;
        properties.uuids = uuids_1.UuidCompletionContribution.propertyMatches;
        const additionalCompletionProvider = this.findAutoCompletionProperty(document, textDocumentPosition, properties);
    ...
    getUuidCompletions = () => {
        const uuidCompletion = new uuids_1.UuidCompletionContribution();
        ...
        const completion = vscode_languageserver_protocol_1.CompletionItem.create(generatedUuid);
        completion.detail = "Generate UUID";
        completion.kind = vscode_languageserver_protocol_1.CompletionItemKind.Function;
    ...
    findAutoCompletionProperty = (document, textDocumentPosition, properties) => {
        let currentLine = textDocumentPosition.line;
        while (currentLine >= 0) {
            const lineOffsets = (0, arrUtils_1.getLineOffsets)(document.getText());
            const start = lineOffsets[currentLine];
            let end = 0;
            ...
            const isOtherItemInList = thisLine.match(/-\s*([-"\w]+)?(\.)?([-"\w]+?)?\s*$/);
            if (isOtherItemInList) {
                currentLine -= 1;
                continue;
            }
            for (const key in properties) {
                if (properties[key].some((propertyName) => 
                // eslint-disable-next-line no-useless-escape
                new RegExp(`(.*)${propertyName}(:)([\s]*)([\w]*)(\s*)`).test(thisLine))) {
                    return key;
                }
            }
            return null;
...
exports.HomeAssistantLanguageService = HomeAssistantLanguageService;
//# sourceMappingURL=haLanguageService.js.map